...
class TicketInUseError < StandardError
  attr_accessor :ticket

  def initialize(ticket)
    @ticket = ticket
    super("Ticket №#{@ticket.id} already in use")
  end
end
...
module TicketsBuyer
  ...
  def buy_ticket(ticket)
    raise TicketInUseError.new(ticket) if ticket.user
    ticket.user = self
    @tickets += [ticket]
  end
...
begin
  user.buy_ticket(ticket)
rescue TicketInUseError => e
  p e.message
end
