...
class Product
  ...
  def check
    raise ErrorSpoiled if @storage_life < 0
    rescue ErrorSpoiled => e
      puts e.message
      puts e.action
    rescue NoMethodError
      puts "not found storage life for product"
    rescue Exception => e
      puts "hm.. undefined error"
  end
  
end

class ErrorSpoiled < StandardError
  def initialize(msg="this product is spoiled")
    super
  end
...
begin
  object_test = Product.new
  object_test.new_method
rescue NoMethodError
  object_test.instance_eval do
    def new_method
      puts 'It is work now'
    end
  end
...
#========================================instead errors, example2

Seller.new_method
Seller.new.new_method

#========================================work with errors, example3

product1.check
product1.storage_life = -1
product1.check